...
{
    public interface IT1ContractRepository
    {
        Task<IEnumerable<T1Contract>> GetContractsByCustomerIdAsync(int customerId);
        Task<IEnumerable<T1Contract>> GetContractsByFilterDataAsync(int locationId, int contractStatusId, bool isDaily, string searchString);
